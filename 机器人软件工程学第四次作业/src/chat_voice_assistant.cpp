...
void voiceWordsCallback(const std_msgs::String::ConstPtr& msg)
{
	...
	else if(dataString.compare("你几岁了？") == 0)
    {
        char helpString[40] = "我今年十岁";
        text = helpString;
        std::cout<<text<<std::endl;
    }
	else if(dataString.compare("你好。") == 0)
    {
		if((float)rand()/RAND_MAX > 0.5){
        	char helpString[40] = "你好";
        	text = helpString;
        	std::cout<<text<<std::endl;
		}
		else{
			char helpString[40] = "哈喽";
        	text = helpString;
        	std::cout<<text<<std::endl;
		}
    }
	...
	else if(dataString.compare("再见。") == 0 || dataString.compare("拜拜。") == 0)
    {
		if((float)rand()/RAND_MAX > 0.5){
        	char helpString[40] = "拜拜";
        	text = helpString;
        	std::cout<<text<<std::endl;
		}
		else{
			char helpString[40] = "再见";
        	text = helpString;
        	std::cout<<text<<std::endl;
		}
    }
...
{
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = 5e7575f4, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
